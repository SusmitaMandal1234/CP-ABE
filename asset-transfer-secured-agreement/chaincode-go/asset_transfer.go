...
func (s *SmartContract) CreateAsset(ctx contractapi.TransactionContextInterface, assetID, publicDescription string, clientOrgID string) error {
	asset := Asset{
		ObjectType:        "asset",
		ID:                assetID,
		OwnerOrg:          clientOrgID,
	...
	// Persist private immutable asset properties to owner's private data collection
	// collection := buildCollectionName(clientOrgID)
	// err = ctx.GetStub().PutState(asset.ID, immutablePropertiesJSON)
	// if err != nil {
	// 	return fmt.Errorf("failed to put Asset private details: %v", err)
	// }
	return nil
...
func setAssetStateBasedEndorsement(ctx contractapi.TransactionContextInterface, assetID string, orgToEndorse string) error {
	//endorsementPolicy, err := statebased.NewStateEP([]byte(`{"version":1,"rule":"nOutOf(2,'Org1MSP.peer','Org2MSP.peer','Org3MSP.peer')"}`))
	...
	fmt.Println("key based policy", string(policy))
	if err != nil {
...
func (s *SmartContract) GetAssetStateBasedEndorsement(ctx contractapi.TransactionContextInterface, assetID string) string {
	policy, err := ctx.GetStub().GetStateValidationParameter(assetID)
	if err != nil {
		return fmt.Sprintf("Error from getting the state" + err.Error())
	}

	return fmt.Sprintf("Policy from the key  is %s ", string(policy))
}
