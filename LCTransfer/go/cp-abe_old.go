...
func ConvertMessage() (output1 string, output2 string) {
	...
	//fmt.Println("msg: ", `{"Name":"Pranay","Email":"PranayChawhan2015@gmail.com","Policy":"(Cement_concrete AND Suppliers_Raw_materials AND (Electronic_Machinary OR Sand OR Cement) )"}`)
	//fmt.Println(string(message))

	message, _ := os.ReadFile("data.json")

	var msg1 map[string]string
	//json.Unmarshal([]byte(string(`{"Name":"Pranay","Email":"PranayChawhan2015@gmail.com","Policy":"(Cement_concrete AND Suppliers_Raw_materials AND (Electronic_Machinary OR Sand OR Cement) )"}`)), &msg1)
	json.Unmarshal(message, &msg1)
