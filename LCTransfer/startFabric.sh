...
./network.sh deployCC -ccn LC-Transfer -ccv 1 -ccl ${CC_SRC_LANGUAGE} -ccp ${CC_SRC_PATH} -cci InitLedger

#For generating the connection-profiles
cd ../test-network/organizations/
./setup.sh

#For setting up the explorer
cd ../../explorer
docker-compose up -d
