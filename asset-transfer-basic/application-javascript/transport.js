...
const { Gateway, Wallets, HsmX509Provider, Transaction,QueryHandlerFactory, DefaultQueryHandlerStrategies
...
let endorsers = network.getChannel('mychannel').getEndorsers().filter(x=>x.name.startsWith("peer0.org1.example.com")
|| x.name.startsWith("peer1.org1.example.com"));




contract.createTransaction("ChangeCarOwner").setEndorsingPeers(endorsers).submit("CAR9","Pranay")

//let result = await contract.submitTransaction('TestMultiChaincodeFeature')
...
await contract.createTransaction("ChangeCarOwner").setEndorsingPeers(endorsers).submit("CAR9","Pranay")

await contract.createTransaction("ChangeCarOwner").submit("CAR9","Balaraju")
